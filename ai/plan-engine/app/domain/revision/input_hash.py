...
고정해야 하므로, 아래 네 가지를 여기서 확정합니다. 하나라도 어긋나면 Java와 Python이 다른 해시를
내고, 리비전이 매 틱 올라가거나 반대로 영원히 안 올라갑니다.

1. **키 정렬과 공백** — 키는 사전순, 구분자는 공백 없는 ``,``·``:``.
2. **좌표** — 부동소수 표기는 언어마다 다릅니다(`0.1` vs `0.10000000000000001`).
   소수점 6자리 고정 문자열로 절단해 넣습니다. 6자리는 약 0.11m로, 지오펜스 반경 100~200m
   기준에서 같은 결정을 낳는 범위입니다.
