...
**문서 역할:** PRD의 제품 범위, TRD의 기술 순서, API 명세의 구현 계약을 하나의 실행 계획으로 묶은 개발 기준 문서

**기준 문서:** 신 PRD (제품 요구사항 정의서) · 신 TRD (기술 요구사항 문서)  · API 명세서
...
- [x]  타 사용자 자원 접근을 404로 처리하는 행 수준 접근 테스트를 추가한다.

### Frontend

- [ ]  로그인, 약관, 이메일 인증 안내, Secure Storage 세션 관리를 구현한다.
- [ ]  홈·지도·캘린더·설정 4탭 스캐폴딩을 만든다.
- [ ]  `/me/bootstrap` 응답으로 초기 화면을 그리는 상태 구조를 확정한다.
- [ ]  API 공통 오류, 재시도 가능 여부, 오프라인 상태를 구분해 처리한다.
...
- [ ]  기상청 또는 에어코리아 Provider 최소 1종을 실제 데이터로 연결한다. <!-- 서비스키 발급받음, @Component 전환·실 응답 검증 남음 -->

### Frontend

- [ ]  준비 시간 입력과 맞춤 준비 항목을 한 화면 흐름으로 구성한다.
- [ ]  일반 준비물, 영양제, 기호 품목, 시간 소요 루틴을 등록할 수 있게 한다.
- [ ]  맞춤 항목 입력은 선택 사항이며 건너뛰어도 온보딩을 완료할 수 있게 한다.
- [ ]  계획 카드에 준비 시작, 권장 출발, 목표 도착과 계산 근거를 표시한다.
- [ ]  기본 경로, 최소 도보, 최소 환승 후보를 선택하면 새 계획 리비전을 반영한다.

### AI · Data · 검증

- [x]  골든 01 기본 도착 역산
- [x]  골든 02 강수 가산과 우산 항목
- [x]  골든 03 시간 루틴이 준비 시각을 앞당김
- [x]  골든 04 준비물은 시간에 영향을 주지 않음
- [x]  골든 05 출발 시각 기준 정방향 계산
- [x]  골든 06 제약 충돌 시 `feasible=false`
- [ ]  ArchUnit으로 계산 계층의 Spring, JPA, `Instant.now()` 의존을 차단한다. <!-- 미확인 -->
...
- [ ]  홈에서 다음 일정과 계획 상태를 표시한다.
- [ ]  준비 시작, 미루기, 출발, 제외를 원탭으로 기록한다.
- [ ]  Drift 기반 오프라인 큐를 앱 종료 후에도 유지하고 재연결 시 재전송한다.
- [ ]  준비 시작과 출발 임박 2건을 로컬 알림으로 사전 예약한다.
- [ ]  서버 푸시가 먼저 도착하면 같은 식별자의 로컬 알림을 취소한다.
- [ ]  보정된 계획과 “왜 바뀌었는지”를 홈과 계획 상세에서 보여준다.
...
- [ ]  사용자 맞춤 준비물과 웰니스 행동을 출처가 구분되도록 표시한다.
- [ ]  UV, 미세먼지, 기온, 강수, 수분 관심 항목과 재알림 주기를 설정할 수 있게 한다.
- [ ]  민감 준비 항목은 잠금화면과 푸시에서 일반화된 문구로 치환한다.
- [ ]  선크림 재확인 등 이벤트 알림에 “사용자가 설정한 시간이 지났다면”과 같은 비의료적 표현을 사용한다.
...
- [ ]  탈퇴 후 동의 이력을 제외한 사용자 데이터가 남지 않는다.
...
- [ ]  `feat/ai-personalization` — **이지호**
    ...
    - 리뷰어: **김민형** · 제품 검증: **최수민**
- [ ]  `feat/fe-notification-offline` — **유채연**
